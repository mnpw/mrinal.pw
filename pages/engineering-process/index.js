...
export default function Posts() {
    return (
        <Layout>
            ...
            <p>
                Most of my network building has come from straight up abusing the search engines. Getting in the <Link
                href="https://twitter.com/_mnpw/status/1462778755198320644"
                target="_blank"
                rel="noopener">rabbit holes</Link> and keeping a high level track of them. Not just opening a lot of links, but parsing through the right kind of information. Combing through five year old commit messages, never opening w3schools links, and digging deep within the source code of libraries decaying in the forest of dependency hell. An explicitised version of this can look like building personal knowledge base (<Link
                href="http://ceasarbautista.com/posts/memex_meetup_2.html"
                target="_blank"
                rel="noopener">memex</Link>, anyone?) built from handpicked resources. <em>Good taste</em> and <em>work experience</em> are the properties of this network. One comes from consuming <em>good</em> things, and the other comes from doing <em>a lot</em> of things.
            </p>
            <blockquote>
                <div className="body">
                ...
                <p className="ref">— Jamie Brandon, <Link
                    href="https://www.scattered-thoughts.net/writing/on-bad-advice/"
                    target="_blank"
                    rel="noopener">On bad advice</Link></p>
            </blockquote>
            <p>
            ...
            <p>
                The only way out of this fog was to take somebody else's definition of the problem and work with that. I spent a good amount of time looking at state of the art solutions to find the 'right' direction. <code>Axum</code>, <code>Tide</code>, and <code>Dropshot</code> to name a few. Looking at their evolution gave some much needed <Link
                href="https://rustasync.github.io/team/2018/09/11/tide.html"
                target="_blank"
                rel="noopener">answers</Link>. <code>Dropshot</code> aimed at elegantly exposing REST APIs while <code>Axum</code> focused on modularity by being deeply connected with <code>Tower</code>. All projects were different from each other in many ways. I focused on tracing the similarities between them. This was essential to answer the question of 'What problem a modern web server is supposed to solve?'
            </p>
            <p>
                Everywhere I looked, I saw <code>async</code>. It was common to most web servers and frameworks I looked at. Concurrency wasn't even on my mind when I set out to extend the project. The <code>async/.await</code> rabbit hole was calling my name and I did not hesitate to jump. One thing led to another and I soon found myself tangled in <code>mio</code>'s source code. Polling? Futures? Executor? The heck? I watched Steve Klabnik <Link
                href="https://www.youtube.com/watch?v=NNwK5ZPAJCk"
                target="_blank"
                rel="noopener">nerding out</Link> many times. Ashley Williams came to my rescue with an absolutely gorgeous <Link
                href="https://www.youtube.com/watch?v=aGJTXdXQN2o"
                target="_blank"
                rel="noopener">talk</Link> on the origin philosophy of asynchronous programming. It led me to some historical <Link
                href="https://www.dre.vanderbilt.edu/~schmidt/PDF/reactor-siemens.pdf"
                target="_blank"
                rel="noopener">artifacts</Link> and gave me confidence to open the <Link
                href="https://copyconstruct.medium.com/nonblocking-i-o-99948ad7c957"
                target="_blank"
                rel="noopener">scrolls</Link> of <Link href="https://www.ncameron.org/rfcs/2394" target="_blank" rel="noopener">ancient</Link> <Link
                href="http://aturon.github.io/tech/2016/09/07/futures-design/"
                target="_blank"
                rel="noopener">wisdom</Link>. From <code>axum</code> to <code>tokio</code>. From <code>tokio</code> to <code>hyper</code>. From <code>hyper</code> to <code>mio</code>. From <code>mio</code> to <code>futures</code>. I maintained a log of the fission as I went through it. There was too much to know but I had absorbed enough to figure out the direction I wanted to go in.
            </p>
            <blockquote>
                <div className="body">
                ...
                <p className="ref">— <Link
                    href="https://twitter.com/bodil/status/1308783627908583430"
                    target="_blank"
                    rel="noopener">Bodil</Link></p>
            </blockquote>
            <p>
                I picked up the beautiful idea of constructing software outside-in while watching one of <Link
                href="https://www.youtube.com/c/JonGjengset"
                target="_blank"
                rel="noopener">Jon's</Link> videos. It is akin to the end user flavour of TDD approach. The approach called for defining the ergonomics of the web server. It meant defining how the <Link
                href="https://github.com/tokio-rs/axum#usage-example"
                target="_blank"
                rel="noopener"><code>main</code>function of the end user</Link> would look like, and how would it register the routes and their associated function. A high level structure became apparent through this. The next step was ensuring bare minimum end-to-end functionality. Respond with a "hello world" static body when I type <code>localhost:3000</code> in my browser. Fancy macros for http methods and modularity can come in later. I actively wanted to avoid the trap of making the web server do everything all at once. It was perfectly okay to not handle all the routes and all the edge cases just yet. It can be a very dumb piece of software, as long as a piece of functionality works end to end.
            </p>
            <p>
                A compass calibrated to the north of novelty is a bad compass. It is one of the uncomfortable truths I have come to realise. The way to the unknown waters is through charted routes. In simple words, it means starting with the "hello world" level of complexity and not trying to do everything. With some hacking I got a very <Link
                href="https://github.com/mnpw/webserver-rs"
                target="_blank"
                rel="noopener">simple version</Link> of the web server up and running. Of course, the process was not as linear as I am making it sound here. It was an iterative mish-mosh of the parts mentioned above. And like all side projects, it is a work in progress.
            </p>
            <p>~</p>
            <h3 className={utilStyles.headingLg}>III.</h3>
            <p>
                While working up on a <Link
                href="https://github.com/hyperium/hyper/pull/2837"
                target="_blank"
                rel="noopener">pull request</Link> for <code>hyper</code>, I picked <code>client::get()</code> as the entry point and started digging from there. It led me to <code>HttpConnector</code> and <code>PoolClient</code> types. That eventually led me to <code>HttpInfo</code>, the place where I wanted to be at to add the feature. For my web server project, I studied <code>mini-redis</code>'s server design. I went from <code>server::run()</code> to <code>Handler::run()</code> to <code>Connection::read_frame()</code>. I used the <code>read_frame</code> method as the basis for parsing raw data to http packets for my project.
            </p>
            <p>
                I came across a <Link
                href="https://mitchellh.com/writing/contributing-to-complex-projects"
                target="_blank"
                rel="noopener">brilliant essay</Link> by Mitchell Hashimoto recently — 'Contributing to complex projects'. He talks about 'trace down, learn up' approach, which I found to be remarkably similar to my hacky process.
            </p>
            <blockquote>
                <div className="body">
